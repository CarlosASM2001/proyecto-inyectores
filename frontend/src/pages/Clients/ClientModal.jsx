  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        setErrors(err.response.data.message); // Fixed typo from 'errros'
      } else {
      ...
      <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="bg-gray-50 px-8 py-6 border-b border-gray-100 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-15 h-10 rounded-xl bg-workshop-red flex items-center justify-center text-white shadow-lg shadow-red-900/20">
              <User size={20} />
            </div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          ...
          <div>
            ...
            <input
              type="text"
              required
              className={`w-full px-4 py-3 rounded-xl border font-bold transition-all focus:ring-4 focus:ring-red-500/10 outline-none "border-gray-200 focus:border-workshop-red"`}
              value={formData.name}
              onChange={(e) =>
